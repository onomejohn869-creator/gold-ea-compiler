...
//|   Trend-following pullback EA for XAUUSD (Gold)                  |
//|                                                                    |
//|   Logic:                                                          |
//|   - 200 EMA determines trend direction (price vs EMA)             |
//|   - RSI(14) identifies oversold/overbought pullbacks in the       |
//|     direction of the trend (mean-reversion-in-trend entries)      |
//|   - ATR(14) sets dynamic SL (1.5x ATR) and TP (3x ATR)            |
//|   - Lot size is calculated dynamically so that a stop-loss hit    |
//|     risks exactly RiskPercent of account equity                   |
//|   - Trading is blocked 30 minutes before/after high-impact USD    |
//|     news events using the native MQL5 Economic Calendar API       |
//+------------------------------------------------------------------+
#property copyright "Generated EA - review and test before live use"
...
// INPUTS
//====================================================================
input string   General_Section        = "===== General Settings =====";
input ulong    InpMagicNumber         = 20260723;   // Magic number
input double   InpFixedSlippage       = 20;         // Max slippage (points)
input bool     InpOneTradeAtATime     = true;       // Allow only 1 open position at a time

input string   Trend_Section          = "===== Trend Filter (EMA) =====";
input int      InpEMAPeriod           = 200;        // EMA period
input ENUM_APPLIED_PRICE InpEMAPrice  = PRICE_CLOSE;// EMA applied price

input string   RSI_Section            = "===== Entry Filter (RSI) =====";
input int      InpRSIPeriod           = 14;         // RSI period
input double   InpRSIOversold         = 30.0;       // RSI oversold level (buy pullback trigger)
input double   InpRSIOverbought       = 70.0;       // RSI overbought level (sell pullback trigger)

input string   ATR_Section            = "===== Volatility (ATR) - SL/TP =====";
input int      InpATRPeriod           = 14;         // ATR period
input double   InpATR_SL_Mult         = 1.5;        // SL = ATR * this multiplier
input double   InpATR_TP_Mult         = 3.0;        // TP = ATR * this multiplier

input string   Risk_Section           = "===== Risk Management =====";
input double   InpRiskPercent         = 1.0;        // Risk per trade (% of account equity)
input double   InpMinLot              = 0.01;       // Absolute minimum lot (safety floor)
input double   InpMaxLot              = 50.0;       // Absolute maximum lot (safety cap)

input string   News_Section           = "===== News Filter =====";
input bool     InpUseNewsFilter       = true;       // Enable high-impact news blackout
input int      InpNewsBlockMinutes    = 30;          // Minutes before/after news to block trading
input string   InpNewsCurrency        = "USD";       // Currency to monitor for news (XAUUSD is USD-quoted)
input bool     InpBlockGoldSpecific   = true;        // Also check "XAU"-tagged calendar events if available

input string   Management_Section     = "===== Trade Management (Breakeven / Trailing) =====";
input bool     InpUseBreakeven        = true;        // Enable ATR-based breakeven
input bool     InpUseTrailing         = true;        // Enable ATR-based trailing stop
input double   InpBE_Trigger_ATR_Mult = 1.5;         // Move to breakeven once profit >= this x ATR
input double   InpTrail_Trigger_ATR_Mult = 1.5;      // Start trailing once profit >= this x ATR
input double   InpTrail_Distance_ATR_Mult = 1.5;     // Trail SL this x ATR behind current price
input double   InpTrail_Step_ATR_Mult = 0.5;         // Only move trailing SL in steps of this x ATR
